...
class GTF:
    ...
    def _parse_gencode(self, df: list, feature: str = "transcript") -> list:
        ...
        if feature == "transcript":
            ...
            df["StartEnd_NF"] = (
                df["tag"].str.contains("cds_start_NF|cds_end_NF|mRNA_start_NF|mRNA_end_NF", na=False).astype(int)
            )
            df["RNA_supported"] = (
                df["tag"]
                .str.contains("nested_454_RNA_Seq_supported|RNA_Seq_supported_only|RNA_Seq_supported_partial", na=False)
                .astype(int)
            )
            for feature in ["basic", "CCDS", "NAGNAG", "readthrough"]:
                df[feature] = df["tag"].str.contains(feature, na=False).astype(int)
            for feature in ["nonsense_mediated_decay", "non_stop_decay", "pseudogene"]:
                df[feature] = df["transcript_type"].str.contains(feature, na=False).astype(int)
        return df
...
def load_appris(filepath: str) -> list:
    ...
    df["crash_p"] = (
        df["crash_p"].replace("-", df.loc[~df["crash_p"].str.contains("-$")]["crash_p"].median()).astype("float64")
    )
    df["crash_m"] = (
        df["crash_m"].replace("-", df.loc[~df["crash_m"].str.contains("-$")]["crash_m"].median()).astype("float64")
    )
    df["tsl"] = df["tsl"].replace("-", 6)
    ...
    df = df.drop(
        ...
        ["translation", "crash", "inertia", "start_stop_codons", "proteo"],
        axis=1,
...
def load_phylocsf(filepath: str) -> list:
    ...
    scorecodon = (
        pd.concat([pd.Series(row["ScorePerCodon"], row["Transcripts"].split(",")) for _, row in df.iterrows()])
        .reset_index(name="ScorePerCodon")
        .rename(columns={"index": "transcript_id"})
        .groupby(["transcript_id"])["ScorePerCodon"]
        .min()
        .reset_index()
    )
    relbranch = (
        pd.concat([pd.Series(row["RelBranchLength"], row["Transcripts"].split(",")) for _, row in df.iterrows()])
        .reset_index(name="RelBranchLength")
        .rename(columns={"index": "transcript_id"})
        .groupby(["transcript_id"])["RelBranchLength"]
        .min()
        .reset_index()
    )
    psi = (
        pd.concat([pd.Series(row["PhyloCSF_Psi"], row["Transcripts"].split(",")) for _, row in df.iterrows()])
        .reset_index(name="PhyloCSF_Psi")
        .rename(columns={"index": "transcript_id"})
        .groupby(["transcript_id"])["PhyloCSF_Psi"]
        .min()
        .reset_index()
    )
...
def load_reference(filepath: str) -> list:
    ...
    """
    df = pd.read_csv(
        filepath, compression="gzip", sep="\t", names=["gene_id", "transcript_id", "ann_type", "rtag", "transcript_ref"]
    )
