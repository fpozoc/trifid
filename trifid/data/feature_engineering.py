...
def load_data(config: dict, assembly: str, release: str) -> pd.DataFrame:
    ...
    try:
        if "gtf" in annotation_path:
            df_annotation = loaders.load_annotation(annotation_path, db=release[0])
    ...
    try:
        if "corsair_alt" in corsair_alt_path:
            ...
            df_corsair_alt = pd.DataFrame(columns=["transcript_id", "corsair_alt"])
    ...
    try:
        ...
        print(
            "Please, provide a right QPfam filepath in the config file. Visit https://gitlab.com/fpozoc/qpfam for more info."
        )
    ...
    try:
        if "qsplice" in qsplice_path:
            ...
            df_qsplice = pd.DataFrame(columns=["transcript_id", "RNA2sj", "RNA2sj_cds"])
        logger.info(utils.get_df_info(df_qsplice))
    except:
        print(
            "Please, provide a right QSplice filepath in the config file. Visit https://gitlab.com/fpozoc/qsplice for more info."
        )
    ...
    try:
        if "PhyloCSF" in phylocsf_path:
            ...
            df_phylocsf = pd.DataFrame(columns=["transcript_id", "ScorePerCodon", "RelBranchLength", "PhyloCSF_Psi"])
    ...
    try:
        if "qduplications" in reference_path:
            ...
            df_reference = pd.DataFrame(columns=["transcript_id", "ann_type", "transcript_ref"])
    ...
    try:
        if "fa" in sequences_path:
            ...
            df_sequences = pd.DataFrame(columns=["transcript_id", "sequence"])
        logger.info(utils.get_df_info(df_sequences))
    except NameError:
        print("Please, provide a right FASTA sequences filepath in the config file.")

    df = utils.merge_dataframes(
        df_appris, df_sequences, df_annotation, df_corsair_alt, df_reference, df_qsplice, df_qpfam, df_phylocsf
    )
