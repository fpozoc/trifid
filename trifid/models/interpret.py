...
from rfpimp import oob_dropcol_importances, dropcol_importances, cv_importances
 
from ..utils.utils import merge_dataframes, get_id_patterns
...
class TreeInterpretation(Splitter):
    ...
    def cv_importances(self):
        ...
        df = cv_importances(
            self.model, 
            self.train_features, 
            self.train_target, k=5).reset_index(
    ...
    def dropcol_importances(self):
        ...
        '''
        df = oob_dropcol_importances(self.model, self.train_features, self.train_target
                                    ).reset_index().rename(
    ...
    def feature_importances(self):
        ...
        df = pd.DataFrame(
            ...
            index=self.train_features.columns).reset_index(
    ...
    def feature_importance_permutation(self):
        ...
        df = pd.DataFrame(
            [imp_mean, imp_std], 
            columns= self.train_features.columns).T.reset_index().rename(
            columns={
                'index':'feature',
                0:'feature_importance_permutation',
                1:'std'
            })
    ...
    def mutual_information(self):
        ...
        '''
        df = pd.DataFrame(mutual_info_classif(self.train_features, self.train_target, random_state=self.random_state), 
                             index=self.train_features.columns).reset_index(
        ).rename(columns={'index':'feature', 0:'mutual_information'}).sort_values(
    ...
    def oob_dropcol_importances(self):
        ...
        df = oob_dropcol_importances(self.model, self.train_features, self.train_target
            ...
            columns={'Feature': 'feature',
                     'Importance': 'oob_dropcol_importances'})
        return df
    ...
    def permutation_importances(self):
        ...
        [permutation_importance.feature_importances_ , permutation_importance.feature_importances_std_], 
            ...
            'index':'feature',
            0:'permutation_importance',
            1:'std'
        })
        df = df[['feature', 'permutation_importance']].sort_values(
    ...
    def target_permutation(self):
        ...
        df = pd.DataFrame(list(zip(self.train_features.columns, bench_mean, perm_mean)),
                                              columns=['feature', 'mdi', 'mda_target'])
    ...
    def local_explanation(self, df_features, sample:str, waterfall:bool=False)->object:
        ...
        if idx == 'transcript_id':
            ...
            df = pd.DataFrame(
                list(zip(
                    np.abs(shap_values).mean(0)[0],
                    df_sample.values[0])
                    ),
                columns=['shap', 'feature'], index=df_sample.columns).sort_values(
