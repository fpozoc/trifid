...
def main():
    ...
    parser.add_argument(
        "-c", "--custom", action="store_true", default=False, help="Training and saving a customized model."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-p", "--pretrained", action="store_true", default=False, help="Train TRIFID with a previously trained model."
    )
    parser.add_argument(
        "-s", "--seed", default=123, help="Perform a nested cv model selection, training and saving the best model."
    )
    ...
    df_features = pd.read_csv(
        os.path.join("data", "genomes", "GRCh38", "g27", "trifid_db.tsv.gz"), sep="\t", compression="gzip"
    )
    ...
    df_training_set = (
        df_training_set.loc[df_training_set["added"].str.contains("v1|r|v3")]
        .drop(["added", "state", "comment"], axis=1)
        .reset_index(drop=True)
    )
    df_training_set.to_csv(
        os.path.join("data", "model", "training_set_final.g27.tsv.gz"), sep="\t", compression="gzip", index=None
    )

    if args.model_selection:
        ms = ModelSelection(
            df_training_set, features_col=df_training_set[features], target_col="label", random_state=args.seed
        )
        model = ms.get_best_model(outdir="models")

    elif args.custom:
        custom_model = RandomForestClassifier(
            n_estimators=400, class_weight=None, max_features=7, min_samples_leaf=7, random_state=args.seed
        )
        model = Classifier(
            model=custom_model,
            df=df_training_set,
