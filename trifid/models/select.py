...
from sklearn.metrics import (accuracy_score, roc_auc_score, average_precision_score, confusion_matrix,
                             balanced_accuracy_score, f1_score, log_loss, matthews_corrcoef, make_scorer,
                             precision_score, recall_score, classification_report, classification_report)
from ..utils import utils


class Splitter():
    
    def __init__(self, df:list, features_col:list, target_col:str, 
                random_state:int, test_size:float=0.25, split_by_gene:bool=False):
        self.df = df
...
class Classifier(Splitter):
    def __init__(self, model:object, df:list, features_col:list, target_col:str, 
                random_state:int=123, test_size:float=0.25, preprocessing:object=None):
    ...
    def cross_validate(self, n_splits:int=5)->dict:
        ...
        cv = cross_validate(
            self.model, df[self.features_col], 
            df[self.target_col], cv=n_splits, scoring=scoring_functions, 
    ...
    def _metrics(self):
        metrics = {'Accuracy': accuracy_score(self.test_target, self.predictions),
...
class ModelSelection(Splitter):
    def __init__(self, df:list, features_col:list, target_col:str,
                random_state:int=123, n_outer_splits:int=5, 
                n_inner_splits:int=10, n_jobs:int=20, save:bool=False,
                filepath:str=None):
        """Inherits train/test/split functionality from Splitter class.
    ...
    def get_best_model(self, outdir:str=None, selection_metric='MCC')->str:
        ...
        for model, config in self._model_configurations().items():
            gcv = GridSearchCV(estimator=config['model'],
        ...
        for model, _ in sorted(gridscv.items()):
            ...
            for i, (train_idx, valid_idx) in enumerate(self._outer_cv().split(self.train_features, self.train_target)):
                ...
                best_model = gcvhp.best_estimator_.fit(self.train_features.values[train_idx], self.train_target.values[train_idx])
                scores = self._evaluate(best_model, self.train_features.values[valid_idx], self.train_target.values[valid_idx])       
        ...
        selected_model = self._model_configurations()[f'{best_model.index[0]}']['model'].set_params(**best_params).fit(self.train_features, self.train_target)
    ...
    def save_results(self, outdir:str):
        self.results.to_csv(os.path.join(outdir, f'model_selection_{datetime.datetime.now().isoformat()}.tsv.gz'), 
                            index=None, compression='gzip', sep='\t')
    ...
    def _model_configurations(self):
        grid = {
            ...
            'Decision Tree': {
                ...
                'grid1': [{'max_depth': list(range(1, 10)) + [None],
            ...
            'Random Forest': {
                ...
                'grid1': [{'min_samples_leaf': list(range(5, 15)),
    ...
    def _get_cv_results(self, cv_results_:dict, metric_sort:str='MCC')->list:
        ...
        cv_results_dict = df[['params'] + [col for col in df if col.startswith('mean_t')]].sort_values(
            by=f'mean_test_{metric_sort}', ascending=False).to_dict('records')[0]
