...
def make_predictions(features:list, ids:list, config:dict, model:object, assembly:str, release:str):
    ...
    if assembly == "GRCh38" and release.startswith("g"):
        ...
        df_features = df[features]
        features_missing = list(df_features[df_features['refseq'] == 'n']['feature'].values) + ['RNA2sj_cds', 'norm_RNA2sj_cds']
        df.loc[df['ccdsid'].str.contains('CCDS'), 'CCDS'] = 1
        df.loc[df['ccdsid'].str.contains('-'), 'CCDS'] = 0
        df_predictions = df[ids]
        ...
        df_acds = pd.read_csv(os.path.join('data', 'external', 'ACDS', 'Rnor_6.0', 'e104', 'acds.tsv.gz'), sep='\t', compression='gzip')
    ...
    labels = ['gene_id', 'gene_name', 'transcript_id', 'translation_id', 'flags',
            'ccdsid', 'appris', 'ann_type', 'length', 'trifid_score', 'norm_trifid_score']
    df_predictions[labels].to_csv(os.path.join(data_dir, 'trifid_predictions.tsv.gz'), index=None, sep='\t', compression='gzip')
...
def main():
    ...
    parser.add_argument('-c', '--config',
                    default='config/config.yaml', help="Configuration filepath.")
    parser.add_argument('-f', '--features',
                    default='config/features.yaml', help="Features selected description filepath.")
    parser.add_argument('-m', '--model',
                    default='models/trifid.v_1_0_4.pkl', help="Features selected description filepath.")
