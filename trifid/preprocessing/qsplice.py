...
def main():
    ...
    parser.add_argument("--experiment", "-e", help="Output directory.", type=str, default="emtab2836")
    parser.add_argument("--file", "-f", help="Custom splice junctions file (in gzip)", type=str)
    parser.add_argument("--gff", "-g", help="Custom or not GENCODE/Ensembl gff file.", type=str)
    ...
    parser.add_argument(
        "--samples", "-s", help="Directory which contains the files to be globbed and concatenated", type=str
    )
    parser.add_argument(
        "--tissue_ids", "-t", help="Directory which contains the files to be globbed and concatenated", type=str
    )
    parser.add_argument("--version", "-v", help="Genome annotation version. GENCODE: `g` + `nversion`", type=str)
    ...
    if args.file:
        ...
        df_sj = concat_samples(
            indir=f"{args.samples}/*/SJ.out.tab", tissue_ids=args.tissue_ids
        )  # concatenating several SJ.out after being annotated and parsed in same pandas DataFrame
        df_sj["tissue"] = (
            df_sj["tissue"].str.split("_").str[0]
    ...
    if args.rm == False:
        df_sj_max_position.to_csv(
            os.path.join(args.outdir, f"sj_maxp.{args.experiment}.tsv.gz"), index=None, sep="\t", compression="gzip"
        )
        df_sj_max_tissue.to_csv(
            os.path.join(args.outdir, f"sj_maxt.{args.experiment}.tsv.gz"), index=None, sep="\t", compression="gzip"
        )

    df_junction_score = score_per_junction(df_introns, df_sj_max_position)
    df_junction_score.to_csv(
        os.path.join(args.outdir, f"sj_maxp.emtab2836.mapped.tsv.gz"), sep="\t", index=None, compression="gzip"
    )

    df_qsplice = score_per_transcript(df_junction_score)
    df_qsplice.to_csv(
        os.path.join(args.outdir, f"qsplice.{args.experiment}.tsv.gz"), sep="\t", index=None, compression="gzip"
    )
...
def annotate_introns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_annot["ncds"] = df_annot.groupby("transcript_id")["cds_coverage"].transform(
        lambda x: x.str.contains("full|partial").sum()
    )
    df_annot = df_annot.sort_values(
        by=["gene_id", "transcript_id", "start", "end"], ascending=[False, True, True, True]
    ).reset_index(drop=True)
    df_annot.loc[df_annot["cds_coverage"].isnull(), "l_intron_coverage"] = df_annot.groupby("transcript_id")[
        "cds_coverage"
    ].shift(1)
    df_annot.loc[df_annot["cds_coverage"].isnull(), "r_intron_coverage"] = df_annot.groupby("transcript_id")[
        "cds_coverage"
    ].shift(-1)
    df_annot.loc[
        (df_annot["cds_coverage"].isnull())
        & (df_annot["l_intron_coverage"].str.contains("none") | df_annot["r_intron_coverage"].str.contains("none")),
        "cds_coverage",
    ] = "none"
    df_annot.loc[
        (df_annot["cds_coverage"].isnull())
        & (
            df_annot["l_intron_coverage"].str.contains("full|partial")
            | df_annot["r_intron_coverage"].str.contains("full|partial")
        ),
        "cds_coverage",
    ] = "full"
...
def load_annotations(filepath: str, db: str, features: list = ["CDS", "exon", "intron"]) -> pd.DataFrame:
    ...
    df_gff = df_gff[
        ["seqname", "type", "start", "end", "strand", "gene_id", "gene_name", "transcript_id", "exon_id", "exon_number"]
    ]
...
def map_junctions_positions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df_sj_max_position = (
        df.sort_values(by=["start", "end", "unique_reads"], ascending=[True, True, False])
    ...
    df_sj_max_tissue = (
        df.sort_values(by=["start", "end", "unique_reads"], ascending=[True, True, False])
        .drop_duplicates(subset=["start", "end", "tissue"], keep="first")
        .reset_index(drop=True)
    )
...
def score_per_junction(df_introns: list, df_sj_max_position: list) -> list:
    ...
    df["gene_mean_cds"] = (
        df.loc[df["cds_coverage"] == "full"].groupby("gene_id")["unique_reads"].transform(lambda x: x.mean())
    )
...
def score_per_transcript(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = (
        df.loc[df.groupby("transcript_id")["unique_reads"].idxmin()]
        .sort_values(
            by=["seqname", "gene_id", "transcript_id", "start", "end"], ascending=[True, True, True, True, True]
        )
        .reset_index(drop=True)
    )
    ...
    if df["transcript_id"].values[0].startswith(get_id_patterns()):
        df["gene_id"] = df["gene_id"].str.rsplit(".", 1).str[0]
        df["transcript_id"] = df["transcript_id"].str.rsplit(".", 1).str[0]
    df = df.sort_values(by=["seqname", "gene_id", "transcript_id", "RNA2sj_cds"], ascending=[True, True, True, False])
...
def _add_intron_number(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df = df.sort_values(
        by=["seqname", "transcript_id", "start", "end"], ascending=[True, True, True, False]
    )  # sorting the positions in order to get the proper exon-CDS order in both 5' and 3'.
...
def _get_exons_cds(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df = df.rename(columns={"start_x": "start", "end_x": "end", "start_y": "start_cds", "end_y": "end_cds"})
    ...
    df.loc[
        (df["start_cds"] != np.nan) & ((df["start_cds"] != df["start"]) | (df["end_cds"] != df["end"])), "cds_coverage"
    ] = "partial"
...
def _parse_emtab(inpath: str) -> dict:
    ...
    df = (
        df[["Comment[ENA_RUN]", "Source Name"]]
        .drop_duplicates(subset="Comment[ENA_RUN]")
        .sort_values(by="Comment[ENA_RUN]")
        .reset_index(drop=True)
