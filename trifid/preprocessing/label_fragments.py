...
def main():
    ...
    parser.add_argument("--gtf", "-g", help="Custom or not GENCODE/Ensembl gtf file.", type=str)
    parser.add_argument("--outdir", "-o", help="Output directory.", type=str)
    parser.add_argument("--principals", "-p", help="APPRIS principal list filepath.", type=str)
    parser.add_argument("--seqs", "-s", help="APPRIS/GENCODE FASTA sequences.", type=str)
    parser.add_argument(
    ...
    if args.trifid:
        ...
        df_pc_annotations = pd.read_csv(
            os.path.join(args.outdir, "gencode.pc_annotations.out.tsv"),
            sep="\t",
            names=["gene_id", "transcript_id", "sequence", "length", "ann_label"],
        )
        df_pc_annotations = pd.merge(
            df_pc_annotations, df_trifid[["transcript_id", "appris", "trifid_score"]], on="transcript_id", how="left"
        )
        df_pc_annotations = df_pc_annotations.sort_values(
            by=["gene_id", "appris", "trifid_score"], ascending=[True, True, False]
        )
        df_pc_annotations.to_csv(
            os.path.join(args.outdir, "gencode.pc_annotations.out.tsv"), sep="\t", header=None, index=None
        )
...
def generate_annotations(gtf_path: str) -> pd.DataFrame:
    ...
    df = df.loc[
        df["transcript_type"].str.contains(
...
def get_NR_list(outdir: str) -> str:
    ...
    """
    perl_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../utils", "get_NR_list.pl")
...
def get_seqlen(outdir: str) -> str:
    ...
    Returns:
        str: command line order to run the program.
    """
    perl_script = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../utils", "get_seqlen.pl")
    cmd = f"perl {perl_script} {outdir}"
    return cmd
