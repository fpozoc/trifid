...
import argparse, functools, os, warnings
...
from ..utils.utils import merge_dataframes, open_files, create_dir, get_id_patterns


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--appris', '-a', 
    ...
    parser.add_argument('--seqs', '-s', 
                        ...
                        type=str)
    parser.add_argument('--spade', '-p', 
                        help='Spade file to be included in the pipeline.', 
                        type=str)
...
def annotation_reference(appris_path:str, fasta_path:str, outdir:str=None, save:bool=None):
    ...
    df_pfam_effects_reference = df_priority.drop_duplicates('gene_id')[['gene_name', 'gene_id', 'transcript_id', 'appris']].reset_index(drop=True)
    df_reference.loc[df_reference['transcript_id'].isin(df_priority['transcript_id']), 'pfam_effects_msa'] = 'Reference'
    df_reference.loc[~df_reference['transcript_id'].isin(df_priority['transcript_id']), 'pfam_effects_msa'] = 'Transcript'
    df_appris_msa = df_reference[['gene_id', 'transcript_id', 'pfam_effects_msa', 'appris', 'sequence']]
    
    if save:
        df_pfam_effects_reference.to_csv(os.path.join(outdir, 'spade_references.tsv'), sep='\t', index=None, header=None)
        df_appris_msa[['gene_id', 'transcript_id', 'sequence']].to_csv(os.path.join(outdir, 'pc_translations.tsv'), sep='\t', index=None, header=None)
...
def load_pfam(file:str) -> pd.DataFrame:
    ...
    df = pd.read_csv(file, sep='\t', 
        names=['gene_id', 'alternative_principal(;sep)','Pfam_domain_number', 
        'Pfam_domain_name', 'Pfam_domain_start', 'Pfam_domain_end', 'Event_type', 
        'Lost_residues_total', 'Lost_residues_pfam', 'State', 'Identity_percent', 
        'Gaps_percent', 'pfam_score'])
    ...
    lost_gain = merge_dataframes(
        lost_residues_total, gain_residues_total, lost_residues_pfam, gain_residues_pfam, 
        on_type='transcript_id', how_type='outer', pivot_on=0, nimpute=0)
    df = merge_dataframes(
        impacts, states, nevents, minpfamscore, lost_gain, 
        on_type='transcript_id', how_type='left', pivot_on=0)
...
def make_spade(spade_path:str, outdir:str) -> pd.DataFrame:
    ...
    if spade_path.endswith('.gtf.gz'):
        ...
        df_spade = pd.read_csv(spade_path, names=['transcript_id', 'hmm_name', 'pep_start',	'pep_end', 'score'], sep='\t')
...
def qpfam_effects(df_reference:list, df_spade:list, pfam_effects_filepath:str)->list:
    ...
    df_npfam_domains = df_spade['transcript_id'].value_counts().reset_index().rename(
        columns={'index':'transcript_id', 'transcript_id':'n_pfam_domains'})
    ...
    df = df[['gene_id', 'transcript_id', 'pfam_score', 
            'pfam_domains_impact_score', 'perc_Damaged_State', 
            'perc_Lost_State', 'Lost_residues_pfam', 'Gain_residues_pfam', 
            'pfam_effects_msa', 'appris']]
    return df
...
def _instance_counter(df:pd.DataFrame, column:str) -> pd.DataFrame:
    ...
    for instance in [x for x in df[column].unique() if str(x) != 'nan']:
        if column == 'Event_type':
            df[f'{instance}_{column}'] = ((df[column] == instance) & (
                (df['State'].str.contains('Lost|Damaged')))).astype(int)
        elif column == 'State':
...
def _get_pfam_percentages(df:pd.DataFrame) -> pd.DataFrame:
    ...
    df['pfam_domains_impact_score'] = 1-(
        (df['Damaged_State']+df['Lost_State']) / df['n_pfam_domains_reference']).fillna(1)
...
def _select_reference_priority(df_reference:pd.DataFrame, sorted_list=['']) -> pd.DataFrame:
    ...
    df = df_reference[df_reference.groupby(['gene_id'])['protein_coding'].transform(max) == df_reference['protein_coding']]
...
def _unique_counter(df:pd.DataFrame, column:str, condition:str) -> pd.DataFrame:
    ...
    """    
    if condition == 'Gain':
        df = df.loc[df['Event_type'].str.contains('Insert', na=False)].groupby(
            'transcript_id').agg({column: lambda x: x.unique().sum()}).reset_index()
