...
def config_altair(chart: object, height: int, width: int) -> object:
    ...
    """
    return (
        chart.properties(height=height, width=width, title="")
...
def explain_prediction(df: pd.DataFrame, model: object, features: list, transcript_id: str) -> pd.DataFrame:
    ...
    df_shap_values = pd.DataFrame(
        list(zip(np.abs(shap_values).mean(0)[0])), columns=["shap_value"], index=df_features.columns
    )
    df_shap_values = df_shap_values.sort_values("shap_value", ascending=False)

    print(
        "The functional score predicted by the model for {} is {}".format(
            transcript_id,
            round(
                df_i.iloc[df_i.index.get_level_values("transcript_id") == transcript_id]["trifid_score"].values[0], 3
            ),
        )
    )
    print("The SHAP base value is {}".format(round(explainer.expected_value[1], 4)))
    plot = shap.force_plot(
        explainer.expected_value[1],
        shap_values[1],
        df_features,
        feature_names=None,
        out_names=f"value predicted for {transcript_id}",
        link="identity",
        plot_cmap=["#0099bf", "#ee9c00"],
        matplotlib=True,
        show=False,
        figsize=(20, 3),
        ordering_keys=True,
...
def plot_appris_histogram(df: pd.DataFrame) -> object:
    ...
    opacity = 1
    chart = (
        alt.Chart(source[["appris", "trifid_score"]])
        .mark_bar()
        .encode(
            x=alt.X(
                xcol,
            ...
            color=alt.Color(ycol, legend=None, title="", scale=alt.Scale(domain=names_order, range=color_order)),
            row=alt.Row(
                ycol,
                title="",
                header=alt.Header(
                    labelAngle=0, labelAlign="left", labelFontSize=16, labelFont="Arial", labelFontWeight="normal"
                ),
                sort=names_order,
            ),
        )
    ...
    feature_importances = (
        ...
        .encode(
            x=alt.X(
                xcol,
                axis=alt.Axis(
                    title=f"Top {method} values (% impact on model output)",
                    titleFont="Arial",
                    titleFontWeight="normal",
                    titleFontSize=20,
                ),
            ...
            y=alt.Y(
                ycol,
                axis=alt.Axis(title="", titleFont="Arial", titleFontWeight="normal", grid=True),
                type="nominal",
                sort=alt.EncodingSortField(field=xcol, order="descending"),
                scale=alt.Scale(),
            ),
            opacity=alt.value(opacity),
            color=alt.Color(facetcol, legend=None, title="", scale=palette),
        )
    )

    chart = (
        alt.layer(feature_importances, data=source)
        .facet(
            row=alt.Row(
                facetcol,
                title="",
                header=alt.Header(
                    labelAngle=0, labelFontSize=16, labelFont="Arial", labelOrient="right", labelPadding=20
                ),
        ...
        .configure_axisX(domainWidth=1.5, domainOpacity=1.5, domainColor="gray", labelFontSize=16, labelFont="Arial")
    )
    return chart
...
def plot_learning_curve(
    ...
    viz = LearningCurve(model, cv=cv, scoring=scoring_function, train_sizes=train_size, n_jobs=-1, size=(500, 350))
    viz.fit(features, target)
    viz.ax.set_xlabel("Training samples", fontsize=16)
    viz.ax.set_ylabel(ylabel, fontsize=16)
    viz.ax.legend(loc="lower right", fontsize=16, bbox_to_anchor=(0.5, 0.0, 0.5, 0.5))
...
def plot_prcurve(model: object, X: list, y: list, n_splits: str, seed: int, ax: object, title: str = None):
    ...
    for i, (train_index, test_index) in enumerate(k_fold.split(X, y)):
        ...
        ax.step(recall, precision, lw=1.3, color="#636363", alpha=0.3, where="post")
        y_real.append(ytest)
        y_pred.append(preds)
...
def plot_pulse_comparison(df, x_col="trifid_score", x_axis_name="TRIFID score"):
    ...
    area_args = {"opacity": 0.8, "interpolate": "step", "color": "#2F4F4F", "limit": 1}
    ...
    top_hist = (
        base.mark_bar(**area_args)
        .encode(
            alt.X(x_col, bin=alt.Bin(maxbins=20, extent=xscale.domain), stack=None, title="", axis=alt.Axis(values=[])),
            alt.Y(
                "count()",
                stack=None,
                title="",
                axis=alt.Axis(labelFontSize=26, labelFont="Arial", labelFontWeight="normal"),
            ),
    ...
    right_hist = (
        ...
        .encode(
            alt.Y(
                "PULSE:Q", bin=alt.Bin(maxbins=20, extent=yscale.domain), stack=None, title="", axis=alt.Axis(values=[])
            ),
            alt.X(
                "count()",
                stack=None,
    ...
    chart = (
        (top_hist & (heatmap | right_hist))
        .configure(concat=alt.CompositionConfig(spacing=5), countTitle="Number of isoforms")
        .configure_legend(
            gradientLength=400,
            gradientOpacity=1,
        ...
        .configure_rect(opacity=0.8, binSpacing=0.5)
    )
    return chart
...
def plot_trifid_appris(df):
    ...
    chart = (
        ...
        .encode(
            ...
            y=alt.Y("sum(pct):Q", scale=alt.Scale(domain=(0, 0.45)), axis=alt.Axis(title="", labelFontSize=16)),
            opacity=alt.value(opacity),
            color=alt.Color(
                "appris_label", legend=None, title="", scale=alt.Scale(domain=names_order, range=color_order)
            ),
            row=alt.Row(
                "appris_label",
                title="",
...
def plot_transcript_types_histogram(df):
    ...
    chart = (
        alt.Chart(df[["trifid_score", "flags_mod"]])
        .mark_bar()
        .encode(
            x=alt.X(
                xcol,
            ...
            opacity=alt.value(opacity),
            color=alt.Color(ycol, legend=None, title="", scale=alt.Scale(scheme=palette)),
            row=alt.Row(
                ycol,
                title="",
                header=alt.Header(
                    labelAngle=0, labelFontSize=18, labelFont="Arial", labelFontWeight="normal", labelAlign="left"
                ),
                sort=names_order,
            ),
        )
...
def plot_validation_curve(
    model: object, seed: int, param: str, param_range: list, scoring_function: object, features: list, target: list
):
