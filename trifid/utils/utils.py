...
class Statistics:
    ...
    def _remove_redundants(self, df):
        df = df.loc[df["nr"] == "yes"].loc[df["flags"].str.contains("^protein_coding$")].reset_index(drop=True)
        return df
...
def balanced_training_set(df: pd.DataFrame, seed: int = 1) -> pd.DataFrame:
    ...
    """
    return pd.concat(
        [df[df["label"] == 1], df[df["label"] == 0].sample(df[df["label"] == 1].shape[0], random_state=seed)]
...
def delta_score(
    df: pd.DataFrame, features: list, mode: str = "appris", groupby_feature: str = "gene_id"
) -> pd.DataFrame:
    ...
    for feature in features:
        ...
        if mode == "longest":
            df[f"{feature}_reference"] = df.groupby(["gene_id"])[feature].transform(max)
            df[f"{feature}_delta"] = df[f"{feature}_reference"] - df[feature]
            df[f"{feature}_delta_score"] = (df[f"{feature}_reference"] - df[f"{feature}_delta"]) / df[
...
def fragments_correction(df_iso: pd.DataFrame, features: list) -> pd.DataFrame:
    ...
    for label in appris_labels:
        if label == "Principal":
            df = df_iso.loc[df_iso["ann_type"].str.contains(f"^Redundant {label}$|^{label}$")]
        elif label == "Alternative":
            df = df_iso.loc[
                (df_iso["ann_type"] == "Redundant {}".format(label))
                | (df_iso["transcript_id"].str.contains("|".join(list(df_iso["transcript_ref"].unique()[1:]))))
            ]
...
def generate_training_set(df_features: pd.DataFrame, filepath: str) -> pd.DataFrame:
    ...
    df = (
        pd.merge(df_features, df, how="left", on="transcript_id")
...
def generate_trifid_metrics(
    df: pd.DataFrame, features: pd.DataFrame, model: object, nmax_norm_median: bool = False
) -> pd.DataFrame:
    ...
    df["norm_trifid_score"] = df.groupby("gene_id")["trifid_score"].transform(
        lambda x: 0
        if (x == 0).all()
        else (1 if ((len(set(x)) == 1) & (x >= 0.5).all()) else (x) / (max(nmax_norm_median, x.max())))
    )
...
def get_nr_transcripts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    df = df.loc[
        df["ann_type"].str.contains("^Alternative$|^Alternative Duplication$|^Principal$|^Principal Duplication")
    ]
    nr_transcripts = (
        df.groupby(["gene_id", "sequence"])["trifid_score", "transcript_id"].max().reset_index().transcript_id.values
    )
    return nr_transcripts


def group_normalization(
    df: pd.DataFrame, features: list, nmax: int = 0, nmin: int = 0, groupby_feature: str = "gene_id"
) -> pd.DataFrame:
    """Normalizer by group.
    ...
    for feature in features:
        if feature in ["firestar", "matador3d", "spade"]:
            df[f"norm_{feature}"] = df.groupby(groupby_feature)[feature].transform(
                lambda x: 0
                if (x == 0).all()
                else (1 if (len(set(x)) == 1) else (x - min(nmin, x.min())) / (max(nmax, x.max()) - min(nmin, x.min())))
            )
        else:
            ...
            df[f"norm_{feature}"] = df.groupby(groupby_feature)[feature].transform(
                ...
                if (x == 0).all()
                    ...
                    (x - df[feature].quantile(plow)) / (df[feature].quantile(phigh) - df[feature].quantile(plow))
                    if (len(set(x)) == 1)
                    else (x - min(nmin, x.min())) / (max(nmax, x.max()) - min(nmin, x.min()))
                )
            )
...
def merge_dataframes(
    *args, on_type: str = "transcript_id", how_type: str = "left", pivot_on: int = 0, nimpute: int = None
) -> pd.DataFrame:
    ...
    args = list(args)
    if len(args) <= 1:
        raise ValueError("Merge function has to receive more than one DataFrame inside list argument.")
    if how_type != "left":
        pivot_on == None
    args.insert(0, args.pop(pivot_on))
    df = (
        functools.reduce(lambda left, right: pd.merge(left, right, on=on_type, how=how_type), args)
...
def round_df_floats(df: pd.DataFrame, n: int = 4) -> pd.DataFrame:
    ...
    """
    df[df.select_dtypes(include="float", exclude=None).columns] = df.select_dtypes(include="float", exclude=None).round(
        n
    )
    return df
